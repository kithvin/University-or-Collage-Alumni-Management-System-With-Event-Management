...
/**
 * REST controller for managing {@link com.kithvin.alumni.domain.Job}.
 */
@RestController
    ...
    public ResponseEntity<Job> partialUpdateJob(@PathVariable(value = "id", required = false) final Long id, @RequestBody Job job)
        ...
        Optional<Job> result = jobRepository
            ...
            .map(existingJob -> {
                ...
                if (job.getSalaryDetails() != null) {
                    existingJob.setSalaryDetails(job.getSalaryDetails());
                }
                ...
                if (job.getJobApplyMethod() != null) {
                    existingJob.setJobApplyMethod(job.getJobApplyMethod());
                }
